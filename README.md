...
仓库：`smokycamera/jev-general-tactic-system`。当前修订版本 `0.1.1`（首版 `v0.1.0` 已发布）。这是独立的游戏决策框架，本轮不修改现有酒馆战斗插件。
...
- 指挥官上下级关系、单位唯一归属，进攻／防御／撤退大类到具体战法的分层选择。
- 五档可配置能力、十四维独立风格、十种战法及阶段意图、分工、预备队与备用战法。
...
**本地 AI 是可运行的规则基线。** 高档能力增加评价因素、候选数量、阶段、协作与预案，不增加单位数值或可见信息。当前敌方反应使用可见火力、包围与资源风险的规则近似，没有实现完整敌军搜索树。十种战法是可扩展的首版模板，不能据此认定真实酒馆战斗已经平衡。
...
重启服务即可。默认单次请求 10 秒，决策预算 30 秒。SDK 内部重试关闭，预算与降级由运行器统一管理。没有密钥时不会向 Jev 发出请求。
